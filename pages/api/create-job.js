...
// Accepts "YYYY-MM-DD" or "MM/DD/YYYY" and returns "YYYY-MM-DD" (or null)
...
export default async function handler(req, res) {
  ...
  // Base44 uploads (assumed URLs). Map them to what process-job.js expects.
  const binsr_url = cleanStr(body.binsr_url ?? body.binsr_document ?? body["BINSR Document"]);
  const inspection_url = cleanStr(body.inspection_url ?? body.full_inspection_report ?? body["Full Inspection Report"]);

  // Optional notes
  const notes = cleanStr(body.notes);

  if (!email) return bad(res, "Missing Email Address");
  if (!binsr_url) return bad(res, "Missing BINSR Document (binsr_url)");
